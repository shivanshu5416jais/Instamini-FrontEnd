    ...
    renderPost=(post)=>{
        ...
        return(
            ...
            <div className="card-body">
                ...
                }/post/photo/${post._id}`} alt={post.title}
                onError={i=>i.target.src=`${defaultPost}`}
                className="img-thumbnail mb-3"
                style={{height:"300px",width:"100%",objectFit:"cover"}}/>
            {like?(
                  <h3 onClick={this.likeToggle}>
                  <i className="fa fa-thumbs-up text-seccess bg-dark" style={{padding:"10px",borderRadius:"50%"}}/>{" "}
                  {likes} Likes</h3>
            ):(
                <h3 onClick={this.likeToggle}>
              ...
              <p className="card-text">{post.body}</p>v
              <br/>
              ...
              {isAuthenticated() && isAuthenticated().user._id===post.postedBy._id &&(
                  <>
                   <Link to={`/post/edit/${post._id}`} className="btn btn-raised btn-warning btn-sm mr-5">Update Posts</Link>
